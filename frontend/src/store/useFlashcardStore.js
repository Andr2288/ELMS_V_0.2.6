...
export const useFlashcardStore = create((set, get) => ({
    ...
    getFlashcards: async (categoryId = null, status = null) => {
        ...
        try {
            ...
            console.log(
                `📦 Store: Updated flashcards from server (${res.data.length} cards)`
            );
            if (res.data.length < 10) {
                console.log(
                    `📦 Store: Cards status snapshot:`,
                    res.data.map((card) => ({
                        text: card.text,
                        status: card.status,
        ...
        try {
            const submitData = {
                ...
                examples: flashcardData.examples
                    ? flashcardData.examples.filter((ex) => ex && ex.trim())
                    : [],
            };
            ...
            const message =
                error.response?.data?.message || "Помилка створення картки";
            toast.error(message);
            throw error;
        }
    },
    ...
    updateFlashcard: async (id, flashcardData) => {
        try {
            ...
            const shouldBeInList =
                !currentFilter ||
                (currentFilter === "uncategorized" &&
                    !updatedFlashcard.categoryId) ||
                updatedFlashcard.categoryId?._id === currentFilter;

            if (!shouldBeInList) {
                set({
                    flashcards: get().flashcards.filter(
                        (card) => card._id !== id
                    ),
                });
            }
    ...
    deleteFlashcard: async (id) => {
        try {
            ...
            const message =
                error.response?.data?.message || "Помилка видалення картки";
            toast.error(message);
        }
    },
    ...
    handleExerciseResult: async (
        ...
        try {
            ...
            if (
                ...
                console.log(
                    `📖 Store: Handling exercise result for ${usedWordIds.length} words:`,
                    usedWordIds
                );
            }

            const res = await axiosInstance.post(
                "/flashcards/exercise-result",
                requestData
            );
            ...
            if (res.data.allWords && Array.isArray(res.data.allWords)) {
                console.log(
                    `📖 Store: Updating ${res.data.allWords.length} words in store`
                );
                ...
                set({
                    flashcards: get().flashcards.map((card) => {
                        ...
                        if (updatedWord) {
                            console.log(
                                `📖 Store: Updating card "${card.text}":`
                            );
                            console.log(
                                `   Status: ${card.status} -> ${updatedWord.status || card.status}`
                            );
                            console.log(
                                `   Reading: ${card.isReadingComprehensionExercise} -> ${updatedWord.isReadingComprehensionExercise}`
                            );
                            console.log(
                                `   Sentence: ${card.isSentenceCompletionExercise} -> ${updatedWord.isSentenceCompletionExercise || card.isSentenceCompletionExercise}`
                            );
                            console.log(
                                `   Multiple: ${card.isMultipleChoiceExercise} -> ${updatedWord.isMultipleChoiceExercise || card.isMultipleChoiceExercise}`
                            );
                            console.log(
                                `   Listen: ${card.isListenAndFillExercise} -> ${updatedWord.isListenAndFillExercise || card.isListenAndFillExercise}`
                            );
                            console.log(
                                `   Choose: ${card.isListenAndChooseExercise} -> ${updatedWord.isListenAndChooseExercise || card.isListenAndChooseExercise}`
                            );
                            ...
                            return {
                                ...
                                isReadingComprehensionExercise:
                                    updatedWord.isReadingComprehensionExercise ??
                                    card.isReadingComprehensionExercise,
                                isSentenceCompletionExercise:
                                    updatedWord.isSentenceCompletionExercise ??
                                    card.isSentenceCompletionExercise,
                                isMultipleChoiceExercise:
                                    updatedWord.isMultipleChoiceExercise ??
                                    card.isMultipleChoiceExercise,
                                isListenAndFillExercise:
                                    updatedWord.isListenAndFillExercise ??
                                    card.isListenAndFillExercise,
                                isListenAndChooseExercise:
                                    updatedWord.isListenAndChooseExercise ??
                                    card.isListenAndChooseExercise,
                                status: updatedWord.status ?? card.status,
                                lastReviewedAt:
                                    updatedWord.lastReviewedAt ??
                                    card.lastReviewedAt,
                            };
                        }
                        return card;
                ...
                console.log(
                    `📖 Store: Successfully updated ${res.data.allWords.length} words`
                );
            } else if (res.data.flashcard) {
                // Стандартна обробка для одного слова
                const updatedFlashcard = res.data.flashcard;
                console.log(
                    `📝 Store: Updating single word "${updatedFlashcard.text}"`
                );
                console.log(
                    `   Old status: ${get().flashcards.find((c) => c._id === flashcardId)?.isSentenceCompletionExercise}`
                );
                console.log(
                    `   New status: ${updatedFlashcard.isSentenceCompletionExercise}`
                );
                    ...
                    flashcards: get().flashcards.map((card) =>
                        card._id === flashcardId
                            ? {
                                  ...
                                  // ВАЖЛИВО: Явно оновлюємо всі поля статусів вправ
                                  isSentenceCompletionExercise:
                                      updatedFlashcard.isSentenceCompletionExercise ??
                                      card.isSentenceCompletionExercise,
                                  isMultipleChoiceExercise:
                                      updatedFlashcard.isMultipleChoiceExercise ??
                                      card.isMultipleChoiceExercise,
                                  isListenAndFillExercise:
                                      updatedFlashcard.isListenAndFillExercise ??
                                      card.isListenAndFillExercise,
                                  isListenAndChooseExercise:
                                      updatedFlashcard.isListenAndChooseExercise ??
                                      card.isListenAndChooseExercise,
                                  isReadingComprehensionExercise:
                                      updatedFlashcard.isReadingComprehensionExercise ??
                                      card.isReadingComprehensionExercise,
                                  status:
                                      updatedFlashcard.status ?? card.status,
                                  lastReviewedAt:
                                      updatedFlashcard.lastReviewedAt ??
                                      card.lastReviewedAt,
                              }
                            : card
                    ),
    ...
    getWordsForExercise: async (
        ...
        try {
            ...
            console.log(
                `🚀 Store: Requesting words for ${exerciseType}: limit=${limit}, category=${categoryId}, excluded=${excludeIds.length}`
            );

            const res = await axiosInstance.get(
                `/flashcards/exercise/${exerciseType}?${params.toString()}`
            );

            // СПЕЦІАЛЬНА ОБРОБКА: Reading comprehension
            if (exerciseType === "reading-comprehension") {
                console.log(
                    `📖 Store: Received ${res.data.words.length} learning words for reading comprehension`
                );

                if (res.data.words && res.data.words.length > 0) {
                    console.log(
                        `📖 Store: Learning words already marked as used on backend, updating local store`
                    );
                    ...
                    set({
                        flashcards: get().flashcards.map((card) => {
                            ...
                            if (updatedWord) {
                                console.log(
                                    `📖 Store: Updating "${card.text}" - isReadingComprehension: ${card.isReadingComprehensionExercise} -> ${updatedWord.isReadingComprehensionExercise}`
                                );
                                return {
                                    ...card,
                                    isReadingComprehensionExercise:
                ...
                if (res.data.wasRotationApplied && res.data.allCategoryWords) {
                    console.log(
                        `🔄 Store: Rotation was applied - updating ${res.data.allCategoryWords.length} cards in store`
                    );
                    ...
                    set({
                        flashcards: get().flashcards.map((existingCard) => {
                            const updatedCard = allUpdatedCardsMap.get(
                                existingCard._id
                            );
                            if (updatedCard) {
                                return {
                                    ...existingCard,
                                    isReadingComprehensionExercise:
                                        updatedCard.isReadingComprehensionExercise,
                                    status:
                                        updatedCard.status ??
                                        existingCard.status,
                                    lastReviewedAt:
                                    ...
                                    isSentenceCompletionExercise:
                                        updatedCard.isSentenceCompletionExercise ??
                                        existingCard.isSentenceCompletionExercise,
                                    isMultipleChoiceExercise:
                                        updatedCard.isMultipleChoiceExercise ??
                                        existingCard.isMultipleChoiceExercise,
                                    isListenAndFillExercise:
                                        updatedCard.isListenAndFillExercise ??
                                        existingCard.isListenAndFillExercise,
                                    isListenAndChooseExercise:
                                        updatedCard.isListenAndChooseExercise ??
                                        existingCard.isListenAndChooseExercise,
                                };
                            }
                            return existingCard;
                        }),
                    ...
                    toast.success(
                        "🔄 Цикл Reading Comprehension оновлено - всі слова доступні знову!",
                        {
                            duration: 4000,
                            position: "top-center",
                        }
                    ...
                    console.log(
                        `🔄 Store: Updated flashcards state after rotation`
                    );
                }

                console.log(
                    `📖 Store: Returning ${res.data.words.length} learning words for reading comprehension`
                );
            } else {
                console.log(
                    `🎲 Store: Received ${res.data.words.length} learning words for ${exerciseType} (shuffled by backend):`,
                    res.data.words.map((w) => w.text)
                );
            }
            ...
            if (res.data.words && res.data.words.length > 1) {
                console.log(
                    `🎲 Store: Applying additional frontend shuffle for ${exerciseType}`
                );
                res.data.words = shuffleArray(res.data.words);
                console.log(
                    `🎲 Store: Final shuffled order:`,
                    res.data.words.map((w) => w.text)
                );
            }
            ...
            console.error(
                `❌ Store: Error getting words for ${exerciseType} exercise:`,
                error
            );
    ...
    getWordsWithProgress: async (status = null) => {
        try {
            ...
            const res = await axiosInstance.get(
                `/flashcards/learning/progress${params}`
            );
            return res.data;
        } catch (error) {
    ...
    resetWordProgress: async (flashcardId) => {
        try {
            const res = await axiosInstance.post(
                `/flashcards/learning/reset/${flashcardId}`
            );
    ...
    generateFlashcardContent: async (
        text,
        englishLevel,
    ...
    generateInteractiveDialog: async (words, englishLevel) => {
        try {
            ...
            console.log(
                `🎯 Store: Generating dialog for words: ${wordsString} (backend will randomize)`
            );
    ...
    regenerateExamples: async (cardId) => {
        try {
            const response = await axiosInstance.post(
                `/openai/regenerate-examples/${cardId}`
            );
